    ...
    public class SearchController : Controller
    {
        ...
        public async Task<IActionResult> QuickSearch(string term)
        {
            ...
            var permissions = await PermissionLookupHelper.HasPermissionsAsync(
                _context,
                User,
                new[] { "EMPLOYEES_VIEW", "KPIS_VIEW", "OKRS_VIEW", "DEPARTMENTS_VIEW" });
            ...
            // Each result category has its own permission gate. The action-level
            // attribute intentionally uses OR semantics and only admits callers that
