# Luồng Demo Dự Án Sau Thuyết Trình - Tối Ưu Cho 6 Thành Viên

> Mục tiêu tài liệu: giúp nhóm demo dự án sau phần thuyết trình một cách mượt, tập trung vào chức năng chính trước, sau đó mới mở rộng sang chức năng nâng cao và các điểm nổi bật.

---

## 1. Mục Tiêu Demo

Sau khi thuyết trình xong, phần demo cần chứng minh hệ thống quản lý được **trọn vòng đời KPI/OKR**, nhưng không nên demo dàn trải tất cả màn hình. Chia demo thành 3 lớp ưu tiên:

| Mức ưu tiên | Mục tiêu | Nội dung cần chứng minh |
| --- | --- | --- |
| P0 - Chức năng chính | Bắt buộc demo mượt | Director xem Dashboard/OKR, Manager quản lý và giao KPI, Employee check-in, Manager duyệt, hệ thống tính score/rank/bonus, Report cập nhật |
| P1 - Chức năng nâng cao | Demo nếu còn thời gian | Phân quyền theo vai trò, PendingDirectorReview, Export Excel, notification, audit log |
| P2 - Điểm nổi bật | Dùng để chốt giá trị | AI Insights/Smart Alerts, dữ liệu tập trung, không tính điểm khi chưa duyệt, tự động tạo/cập nhật EvaluationResult |

Thông điệp chính cần lặp lại trong demo:

- Hệ thống không chỉ nhập KPI, mà quản lý trọn vòng đời KPI/OKR.
- Check-in của nhân viên không tự động được tính điểm, mà phải qua bước duyệt.
...
- Phân quyền đảm bảo mỗi vai trò chỉ thấy và thao tác đúng phạm vi.
...
## 3. Phân Chia Demo Tối Ưu Cho 6 Thành Viên

Mặc định demo trong **10-12 phút**. Không cần mỗi người demo quá nhiều màn hình; mỗi thành viên chỉ cần làm rõ một mắt xích trong luồng chính.

| Thành viên | Vai trò thuyết trình | Tài khoản demo chính | Thời lượng | Nội dung demo |
| --- | --- | --- | ---: | --- |
| Thành viên 1 | PO | `test_director` | 1.5-2 phút | Mở bối cảnh, Dashboard toàn công ty, OKR chiến lược |
| Thành viên 2 | SM | `test_hr` hoặc `test_admin` | 1-1.5 phút | Nói rất nhanh dữ liệu nền: nhân sự, phòng ban, kỳ đánh giá, bonus rule |
| Thành viên 3 | Luồng hệ thống | `test_manager` | 2 phút | KPI, liên kết OKR, phân bổ KPI cho nhân viên/phòng ban kèm trọng số |
| Thành viên 4 | Frontend | `test_employee` | 2 phút | Employee xem KPI được giao, tạo check-in và nhấn mạnh trạng thái Pending |
| Thành viên 5 | Backend | `test_manager` | 2.5 phút | ReviewQueue, duyệt check-in, hệ thống cập nhật score/rank/bonus/evaluation |
| Thành viên 6 | Tester | `test_director` hoặc `test_employee` | 1.5-2 phút | Report, phân quyền, AI/Export nếu còn thời gian, kết luận giá trị |

Nếu được demo **18 phút**, mở rộng thêm phần nâng cao: Director review cuối, export Excel, AI Insights, audit log và thử AccessDenied.

### Nguyên Tắc Tối Ưu Khi Demo

- Ưu tiên luồng P0: Dashboard/OKR -> KPI assignment -> Employee check-in -> Manager approve -> Evaluation/Report.
- Không sa vào CRUD nền tảng quá lâu; Employees, Departments, Positions chỉ dùng để chứng minh dữ liệu đã sẵn sàng.
- Không tạo quá nhiều dữ liệu mới trong lúc demo; dùng dữ liệu seed như `TST-Q2-2026`, `TST - Doanh số cá nhân Q2`.
- Nếu thao tác tạo mới lỗi hoặc mất thời gian, chuyển ngay sang dữ liệu mẫu pending `TST_PENDING_EMPLOYEE_FLOW`.
- Phần AI, Export, Audit log là điểm cộng, không để chúng làm chậm luồng chính.
...
flowchart LR
    ...
    E --> F["Tester<br/>Report, phân quyền, AI/Export nếu còn thời gian"]
```
...
7. Nếu còn vài giây, mở nhanh `/EvaluationReports` để cho thấy có báo cáo tổng hợp.
...
7. Không demo tạo/sửa/xóa ở phần này; chỉ chứng minh dữ liệu nền đã sẵn sàng.
...
8. Chỉ ra score/rank/expected bonus nếu màn hình có dữ liệu.
9. Nếu còn thời gian, mở lại `/Dashboard` để nói dữ liệu đã phản ánh vào tổng quan.
...
- `/AI/SmartAlerts` hoặc widget AI nếu còn thời gian
...
7. Nếu thiếu thời gian, bỏ AI và chỉ chốt bằng Report + phân quyền.
