(function () {
    ...
    window.showEditModal = function (id, rankId, bonus, amount) { const modalElement=document.getElementById('editModal'); if (!modalElement) return; const idInput=modalElement.querySelector('#editId'), rankInput=modalElement.querySelector('#editRankId'), pctInput=modalElement.querySelector('#editBonusPercentage'), amountInput=modalElement.querySelector('#editFixedAmount'), error=modalElement.querySelector('#editModalError'); idInput.value=id || ''; rankInput.value=rankId || ''; if (window.AppComboBox && typeof window.AppComboBox.sync === 'function') { window.AppComboBox.sync(rankInput); } pctInput.value=bonus == null || bonus === '' ? '' : String(bonus); amountInput.value=formatNumber(amount); error.hidden = Boolean(rankInput.value); if (!rankInput.value) { error.textContent='Không tìm thấy xếp loại tương ứng; không thể lưu thay đổi an toàn.'; } if (window.bootstrap) window.bootstrap.Modal.getOrCreateInstance(modalElement).show(); };
